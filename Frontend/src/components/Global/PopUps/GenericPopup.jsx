...
const GenericPopup = forwardRef((props, ref) => {
    ...
    const showPopup = useCallback((message, type = 'Info') => {
        ...
        // Actually hide after fade animation completes (0.5s after fade starts)
    ...
    }, [showPopup, timeoutId, fadeTimeoutId]);
    ...
    return (
        <>
            {visible && (
                <div className={`genericpopup-backdrop ${fading ? 'genericpopup-fadeOut' : ''}`}>
                    <div className={`genericpopup-container ${getTypeClass()} ${fading ? 'genericpopup-slideOut' : ''}`}>
