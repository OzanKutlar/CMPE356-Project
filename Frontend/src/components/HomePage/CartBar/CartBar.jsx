...
const CartBar = ({showNavbar, setShowNavbar}) => {
    ...
    const handleSubmitOrder = async () => {
        setSubmitting(true);
        try {
            const response = await Util.callBackend("cart/saveCart", {userID: Util.savedUser.id} ,{items: JSON.stringify(cartItems)});
            ...
            Util.CallGeneric("Failed to submit order. Please try again later.", "Error");
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <div className="mt-4 h-4/5 overflow-y-auto">
                {loading ? (
                    <div className="flex flex-col items-center justify-center h-full">
                        <div
                            className="w-12 h-12 border-4 border-t-red-500 border-gray-200 rounded-full animate-spin"></div>
